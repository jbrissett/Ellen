...
- **MAP-FIRST, THEN GATE BEFORE QCHUB.** When the user asks for "both" / "go" / "make the map and the order" / "let's run everything", DO NOT fire `create_mymaps_map` and `create_qchub_order` in parallel. Instead:
  1. Fire `create_mymaps_map` only — get the map built and the share link landed in chat.
  2. **Pause and explicitly ask the user to confirm the pins.** Phrase it tightly: "Map is up at <link>. Review the pins (especially the medium-confidence ones) — say 'looks good' to proceed to qchub, or edit in MyMaps and drop the corrected KMZ here." This gate is mandatory even when the email shipped with a KMZ — but particularly critical when there was no client KMZ, because every pin is then geocoder-derived and a wrong pin in qchub = a returned trip + lost margin.
  3. ONLY fire `create_qchub_order` after the user explicitly confirms ("looks good" / "proceed" / "go ahead" / etc.) OR after a re-dropped KMZ has been applied and the user signals to continue.
  Why: the map step is cheap to redo, the qchub step is expensive. A misplaced pin caught at the map stage costs 30 seconds; the same pin caught after qchub submission means manually correcting the order or asking the client. Always make the user the gate.
- **Other independent actions can still parallelize** — e.g., if the user asks for both a KMZ export and a map at the same time. The gate above is specific to the map → qchub sequence because qchub commits real data to QC's system.
- **Email-draft always waits** — it needs the qchub estimate PDF to attach, so it runs after qchub completes.
